...
def main() -> None:
    ...
    report = status._build_report(
        records=records,
        queue=queue,
        readiness=_readiness(_target_case_ids(inspiration)),
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--harness-dir", type=Path, default=REPO_ROOT / "artifacts/ae_live_checks/_manual_harnesses_motif_gaps")
    parser.add_argument("--output", type=Path, default=REPO_ROOT / "artifacts/ae_live_checks/_manual_harnesses_motif_gaps/status.json")
    parser.add_argument("--markdown-output", type=Path, default=REPO_ROOT / "artifacts/ae_live_checks/_manual_harnesses_motif_gaps/status.md")
    parser.add_argument("--decisions", nargs="+", default=sorted(DEFAULT_DECISIONS))
    return parser.parse_args()


def _target_case_ids(inspiration: dict[str, Any]) -> list[str]:
    case_ids = []
    seen = set()
...
def _markdown(status_module: Any, report: dict[str, Any]) -> str:
    return status_module._markdown(report).replace(
        ...
        "# Manual AE Motif-Gap Harness Status",
        1,
    )
