...
def main() -> None:
    ...
    targets = _select_targets(records, inspiration, queue, set(args.decisions))
    ...
    _write_index(written, runner, targets, inspiration, output_dir)
    print({
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", type=Path, default=REPO_ROOT / "artifacts/ae_live_checks/_manual_harnesses_motif_gaps")
...
def _select_targets(
    ...
    for gap in inspiration.get("train_ready_gaps", []):
        ...
        if case_id and case_id not in seen and case_id in records_by_id and decision in decisions:
            seen.add(case_id)
